...
abstract class ET_SocialAuth extends ET_Base{
	...
	protected function create_user($params){
		// insert user
		$result = FE_Member::insert( $params );
		...
		try {
			...
			if ( $user == false ){
				...
				if ( isset($auth_info['user_login']) ){
					...
					if ( $user == false ){
						$result = FE_Member::insert($auth_info);
...
class ET_TwitterAuth extends ET_SocialAuth{
	...
	public function __construct(){
		parent::__construct('twitter', 'et_twitter_id', array(
			...
			'content' 		=> __("<p>This seems to be your first time signing in using your Twitter account. </p>
				<p>If you already have an account with ".get_bloginfo('name' ).", please log in using the form below to link it to your Twitter account. Otherwise, please enter an email address and a password on the form, and a username on the next page to create an account.</p>
				<p>You will only do this step ONCE. Next time, you'll get logged in right away.</p>", ET_DOMAIN),
			'content_confirm' => __("Please provide a username to continue", ET_DOMAIN)
		));
		$this->consumer_key 		= et_get_option(self::OPT_CONSUMER_KEY, ''); // 'H7ggzgE4rNubSq09SKQJGw'; 
		$this->consumer_secret 		= et_get_option(self::OPT_CONSUMER_SECRET, ''); //'zUrMVznhHvrMEKBE5LhipfvRODLlPsvEJLvYiaf4yqE';
		...
		// twitter auth callback
 		...
 		// create user 
			...
			if ( !empty($_POST['user_email']) ){
				...
				$result = FE_Member::insert( $new_account );
				if ( !is_wp_error( $result ) ){
					// send email here
					// 
					do_action('et_after_register', $result);
...
class ET_FaceAuth extends ET_SocialAuth{
	...
	public function __construct(){
		parent::__construct('facebook', 'et_facebook_id', array(
			...
			'content' 		=> "<p>This seems to be your first time signing in using your Facebook account. </p>
				<p>If you already have an account with ".get_bloginfo('name').", please log in using the form below to link it to your Facebook account. Otherwise, please enter an email address and a password on the form, and a username on the next page to create an account.</p>
<p>You will only do this step ONCE. Next time, you'll get logged in right away.</p>",
			'content_confirm' => __("Please provide a username to continue", ET_DOMAIN)
		));
	...
	public function add_scripts(){
		...
		wp_localize_script( 'facebook_auth', 'facebook_auth', array(
			'appID' 		=> et_get_option('et_facebook_key'),
