...
class MakePOT {
	...
	var $meta = array(
		'default' => array(
			...
			'last-translator' => "{team-name} <{team-email}>",
		...
		'forum-engine' => array(
			'description' => 'Translation of the WordPress theme {name} {version} by {author}',
			'msgid-bugs-address' => 'http://enginetheme.com',
			'copyright-holder' => '{author}',
			'package-name' => '{name}',
			'package-version' => '{version}',
		)
	...
	function xgettext($project, $dir, $output_file, $placeholders = array(), $excludes = array(), $includes = array()) {
		...
		$pot->set_header( 'Last-Translator', $meta['last-translator'] );
		$pot->set_header( 'Language-Team', $meta['language-team'] );
