...
/**
 * re-generate password for the account 
 *
 * @since 1.0
 */
// function et_reset_password($key){
// 	global $wpdb;
...
// 		$result = $wpdb->update( $wpdb->users, array('user_activation_key' => '' ), array('ID' => $user->ID), array('%s'), array('%d') );
// 		update_user_meta( $user_id, 'et_activation_key' , '' );

// 		do_action('et_after_reset_pass', $user, $new_pass);
